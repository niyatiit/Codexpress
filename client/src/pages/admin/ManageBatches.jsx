            ...
            <div className="breadcrumb-with-buttons mb-24 flex-between flex-wrap gap-8">
              <div className="breadcrumb mb-24">
                <ul className="flex-align gap-4">
                  ...
                  <li>
                    <span className="text-main-600 fw-normal text-15">Manage Batch</span>
                  </li>
                </ul>
              </div>
            </div>
              ...
              <tbody>
                {batches.map((batch) => (
                  <tr key={batch._id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="p-3 text-sm text-gray-700">{new Date(batch.start_date).toLocaleDateString()}</td>
                    <td className="p-3 text-sm text-gray-700">{new Date(batch.end_date).toLocaleDateString()}</td>
                    ...
                    <td className="p-3 text-sm">
                      <Link
                        to={`/admin/edit-batch/${batch._id}`} // Link to edit batch page
                        className="text-blue-600 hover:text-blue-800 mr-2"
                      >
                        Edit
                      </Link>
                      <button className="text-red-600 border-[1px] bg-red-500 text-white px-2 py-1 rounded-md hover:text-red-800">Deactivate</button>
                    </td>
                  </tr>
                ))}
              </tbody>
